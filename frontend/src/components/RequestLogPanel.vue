...
const COL_BOUNDS: Record<ColKey, [number, number]> = {
  ...
  time: [92, 170],
  model: [120, 300],
  // 渠道列下限 130 → 128 → 88（2026-09-26）。
  //
  // 130 是 2026-09-16 那组人工定宽常量的遗留，当时按「最坏情况」拍。
  // 2026-09-23 改成按内容量之后它被原样留了下来，2026-09-24 降到 128 ——
  // 但那两次降法都是「按库里最长渠道名」定下限（CodingPlan，文字宽 82px），
  // 本质还是最坏情况常量，只是拍得小了点：渠道名是全表里最参差不齐的数据
  // （实测 GOAT 只要 37px、CodingPlan 要 82px），下限钉在任何一个大值上，
  // 碰上短名字的页面就是一列空白（站主 2026-09-26 截图反馈的正是这个）。
  //
  // 88 的来历：下限只负责两件事 —— 放得下表头（「渠道」二字 26px +
  // 单元格内边距 16 = 42px）与放得下「图标 + 短名字」的最小形态
  // （图标 18 + 间距 6 + 内边距 16 + 呼吸 6 + 4 字符 38px ≈ 84px）。
  // 「本页出现更长的名字时撑宽」交给测量（max(下限, 内容所需)），
  // 它对当前页的每一行都生效，不需要下限替它兜底。
  // 下限与测量用**同一套加法**（都含 CONTENT_MEASURE 的 pad：18 图标 + 6 间距），
  // 否则下限会稳定地比所需少一个 pad 的量（第一版算成 82 就漏了）。
  //
  // 这里踩过一次判据陷阱，记下来免得重蹈：`.chan-name` 有
  // `text-overflow: ellipsis`，被截断时 `scrollWidth` 会被压到接近
