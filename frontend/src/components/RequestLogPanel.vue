...
const COL_BOUNDS: Record<ColKey, [number, number]> = {
  ...
  // 渠道列下限 130 → 128。
  //
  // 130 是 2026-09-16 那组人工定宽常量的遗留，当时按「最坏情况」拍，
  // 2026-09-23 改成按内容量之后它被原样留了下来 —— 于是成了唯一一个
  // 「下限远高于实际所需」的列：实测（1440 视口、当前页渠道名 D1余额）
  // 内容只要 93px（名字 47 + 图标 24 + 内边距 16 + 呼吸 6），
  // 下限却把它顶到 130，antd 再按比例分余量后渲染成 137px，
  // **44px 全是空白**；同期其它列的空白只有 28-34px。
  //
  // 128 的来历：按**真实数据里最长的渠道名**定，而不是拍一个最坏情况。
  // 库里最长的是 `CodingPlan`（10 字符，离屏实测文字宽 82px），于是
  //   82（名字）+ 24（CONTENT_MEASURE 里 channel 的 pad：18 图标 + 6 间距）
  //   + 16（单元格左右内边距）+ 6（呼吸）= 128
  //
  // 这里踩过一次坑：第一版算成 122，漏了 pad 里的 6px 间距 ——
  // 1280 视口实测 CodingPlan 仍是「需 82px 只有 75px（差 7px）」。
  // 下限与测量必须用**同一套加法**（都含 CONTENT_MEASURE 的 pad），
  // 否则下限会稳定地比所需少一个 pad 的量。
  //
  // 为什么下限必须放得下最长名字，而不指望测量去撑宽：测得宽度大于下限时
  // 确实会接管（`max(下限, 内容所需)`），但 1280 视口实测渠道列曾停在 108px，
  // 名字被压到 74px 触发省略号。下限把必须成立的下界钉死，
  // 测量只负责「出现更长的名字时继续撑」。
  //
  // 顺带记一个判据陷阱：`.chan-name` 有 `text-overflow: ellipsis`，
  // 被截断时 `scrollWidth` 会被压到接近 `clientWidth`
  //（实测 clientW=74 / scrollW=75，而真实文字宽 82）——
  // 所以「scrollWidth ≤ clientWidth」对省略号列**不敏感**，
  // check-log-columns.mjs 原来那条在这列上会漏判；该脚本 2026-09-24
  // 补了「离屏 span 量文字宽」的判据来覆盖它。
  channel: [128, 240],
  tokens: [140, 200],
  elapsed: [120, 150],
  cost: [90, 140],
