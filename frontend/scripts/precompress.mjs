...
//     必须在产物就位之前完成。而「产物怎么来」这件事在仓库里有两条路
//     （release.yml 的构建步骤、verify-release-artifacts.sh 的本地副本），
//     它们唯一的共同点是**都执行 `npm run build`**。挂在这一步，两条路
//     自动全覆盖；挂在任何一条下游，另外一条就会安静地发不压缩的资源。
//   · 压缩级别可以开到最高，代价只是构建慢一点；现压则要每次请求都花这个
//     CPU，而静态资源会被反复请求。
//   · 运行时只做「选文件 + 发字节」，没有压到一半失败这种状态。
//
// 输出必须**可复现**：不写 gzip 头里的文件名与时间戳，同一份输入每次压出
// 完全相同的字节。否则每次构建产物都不同，「内容是否变化」的判断
// 就会失去意义。
//
