...
//
// 管理接口如今有登录鉴权，但在鉴权之前它长期没有登录态 —— 浏览器里的
// **任何**网页都能对 localhost 发起请求。一个
//
...
//
// 有了登录会话后它依然是 CSRF 的第一道防线：会话 Cookie 会随跨站表单
// 提交一起发出去，光靠 SameSite=Lax 挡不住所有老浏览器；这条中间件按
// Origin 拒绝一切来路不符的浏览器请求，与 Cookie 属性构成两道防线。
//
// 判断依据用 Origin 头，理由：
