...
**兜底目标行配了「对外名 → 上游名」映射时，映射照常生效**：`UpstreamName` 用该行
配置的上游名（2026-09-21 修复：此前被硬写成对外名，映射被忽略 —— 见文末评审一节）。
...
3. **兜底目标必须在该渠道白名单里且启用**：关掉或删掉那一行，兜底随之失效。
   保存时拦截（2026-09-21 起连「行在但停用」也拦）；分两步造出的半残
   （先配好、事后停用白名单行）由失败提示点名，渠道列表的「兜底」胶囊
   按与路由同一判据回显，半残不亮。
...
## 评审修复（2026-09-21，全面审评后）

一次系统性评审发现三处缺陷（均已修，端到端第 6b/8b/10 组断言钉住）：

1. **兜底路径忽略兜底目标行的上游名映射**（实证过的路由缺陷）：
   `buildCandidates` 把兜底候选的 `UpstreamName` 硬写成 `DefaultModel`（对外名），
   该行配置的「对外名 → 上游名」映射被忽略 —— 白名单行配了映射、兜底又指向它时，
   上游收到它不认识的名字，复现本功能要消灭的 2ms 502。计费不受影响（计费键是
   `PublicName`），错的是发出去的名字。修复为 `UpstreamName = rw.UpstreamName`，
   与精确命中同一语义。此前所有测试里兜底目标行的 upstream_name 恰好都等于
   public_name，断言对两种实现失明 —— 单测数据已特意改为两者不同。
2. **保存校验不检查兜底目标行的启用状态**：`validateDefaultModel` 只比名字，
   「先选好兜底目标、再回白名单把那一行停用」一次提交就能带进「配了不生效」的
   状态（前端下拉框只列启用行，防不住选择之后的停用）。现在按整行状态校验，
   名字在但停用报「请启用那一行或另选」。
3. **备份导入绕过全部兜底校验 + 列表胶囊撒谎**：导入走 `tx.Create` 直写
   `extra_config`，不经过 `validateDefaultModel`（渠道先建、白名单后插，建渠道时
   也无处校验）；`fallbackModelOf` 又只看开关、不查白名单，半残配置照样亮胶囊。
   现在：导入在白名单落库后做终审（`stripBrokenFallbackConfigs`），配不上的
   剥掉两个键并写进报告；列表胶囊改为按与路由 JOIN 同口径（目标行存在且启用）
   回显，半残不亮。

评审同时确认无冲突的组合：同渠道精确+兜底去重、密钥 `allowed_models` 不豁免、
分组白名单两轮同条件、价格/时段倍率/模型级代理自动继承、探测不走 `Candidates`。

### 已知未修的边界（设计决策，待定后再动）

- **兜底失败污染渠道健康度**：兜底请求失败照常计入渠道失败连击，连续 3 次把整条
  渠道打进冷却 —— 若兜底目标模型被上游下线，殃及该渠道其他模型的精确流量。
  修复方向是失败记账分模型维度，涉及熔断语义，需单独设计。
- **重试预算可能被精确段耗尽**：兜底永远排在精确段之后，精确候选数 ≥ 最大尝试
  次数且全失败时，健康的兜底渠道得不到一次机会。是否给兜底段保留一次尝试，
  待实际踩到再定。
