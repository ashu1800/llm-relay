...
- **看板可按分组 / 渠道筛选**：时间范围右侧可再选分组与渠道，四张概览卡、
  趋势、模型榜、消耗占比、热力图全部按这个范围取数（筛选条件本地持久化，回来还是它）。
...
- **详尽的请求日志**：首包时间、总耗时、渠道与模型映射、状态码、原始报文、计费过程还原、导出；
  列表顶部可按**分组 / 渠道 / 模型 / 时间范围**筛选（列表与导出同一套条件）；
  trace_id 与「只看失败」这两个排障入口不占工具栏，走 URL（`?trace_id=…`、
  `?status_class=error`），或用详情里的「只看这条链路」
...
- **实时推送**：WebSocket 推送看板数值与请求日志，数字用滚动动画过渡、
  日志插到第一行，都不重绘整页（推送固定是「今天 + 全站」的视角；
  选着其它时间范围或筛过之后不会被它改写，那两种视角用「刷新」取数）
