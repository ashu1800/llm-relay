...
- **详尽的请求日志**：首包时间、总耗时、渠道与模型映射、状态码、原始报文、计费过程还原。
  ...
  不再靠界面文案说明。
  trace_id 与「只看失败」这两个排障入口**仍然作用于列表**。「只看失败」在
  列表顶部工具条上有一个「仅失败」切换按钮（2026-09-24 加，此前只能手改
  URL）；trace_id 走 URL（`?trace_id=…`）或用详情里的「只看这条链路」——
...
- **实时推送**：WebSocket 推送看板数值与请求日志，数字用滚动动画过渡、
  ...
  客户端不去重复实现一遍筛选语义。
   ...
   心跳只证明链路活着，**不代表有新数据**，界面上也不刷新「最后更新」
- **新行不顶走正在读的行**：列表滚到中段、鼠标停在表体上、或详情抽屉开着时，
   新日志不再直接插到第一行（一次插入会把正在读的那行整体顶下去一行
   44/45px，鼠标没动而指的那一行已经换人，此时点「详情」打开的可能是
   上一条请求），改为攒起来并在表格上方显示「有 N 条新日志，点击查看」——
   点了才落地并滚回顶部。三种情形都不成立时（停在顶部、鼠标不在、抽屉关着）
   仍然即时插入：那才是「盯着最新发生了什么」的常态（2026-09-24 加，
   另有 `overflow-anchor` 兜底浏览器自身重排导致的内容跳动）
- **实时状态指示**：请求日志面板顶部的工具条左侧显示「实时 · 最后更新 HH:mm:ss」，
   链路断了就转红并换成「已断开，正在重连 · 最后更新 HH:mm:ss」（2026-09-24 加，
   此前 `liveConnected` 导出了却无人使用，WS 断了界面照旧显示旧数据 ——
   「实时」二字因此不可信）。时间戳只在数据或推送真的到达时前进，
   所以它同时回答两件事：这屏数据停在哪个时刻、以及为什么不再动
...
node .shots/verify-delight.mjs              # 观感增强端到端：脉搏条、页签心跳、昨日战报、
                                            # 里程碑彩带、渠道生命灯、主题扩散
node .shots/verify-failonly.mjs             # 「仅失败」切换（面板顶部工具条）：点击后列表按
                                            # status_class=error 重取、工具栏标签同步出现/消失、
                                            # 再点恢复全量、深链 ?status_class=error 进来时为激活态
node .shots/verify-live-status.mjs          # 实时状态行：首屏「实时 · 最后更新 HH:mm:ss」、
                                            # 有推送时时间戳前进、**心跳不刷新它**，
                                            # 并在 75 秒空闲里盯住不许闪「已断开」
...
node .shots/verify-insert-anchor.mjs        # 实时插行不顶走正在读的行：滚到中段/鼠标悬停/
                                            # 抽屉打开时新行挂起（出现「有 N 条新日志」），
                                            # 点了才落地并回顶部；停在顶部时仍即时插入
