...
- **默认模型映射（兜底）**：每条渠道可单独开启，指定一个白名单里的模型。
  ...
  - **日志里请求模型仍记客户端的原名**，上游模型记兜底目标，并带「兜底」标记 ——
  ...
  不会被兜底绕开）；兜底目标必须是该渠道白名单里**启用中**的条目
...
```bash
curl -s http://127.0.0.1:8888/api/admin/system/info | python3 -m json.tool
```
...
管理接口**没有登录鉴权**（`deploy/.env.example` 里默认 `SERVER_HOST=127.0.0.1`
只监听本机就是为此 —— host 网络模式下它就是宿主侧的实际绑定地址）。
绑到 `0.0.0.0` 之前请自行加反向代理与访问控制 ——
